...
priceDate = new Date(nowDate.getTime() - (2 * 60 * 1000));
primaryAccount = {};
primaryEmail = {};
primaryKey = {};
//get new blocks
        ...
        getAccounts(verifiedAccountUrl).then(data => {
            ...
            getAlias().then(data => {
                ...
                getBlockData(results).then(data => {
                    ...
                    console.log("Number of blocks: " + key.length);
			...
			summary = data['blocks'][keyValue];
                        ...
                        if (+summary.amount >= 25000000000000000000000000000000000) {
                            ...
                            try {
                                ...
                                twitterClient.post('statuses/update', { status: start + from + to + (+summary.amount / 1000000000000000000000000000000).toFixed() + ' $NANO transfer -  https://nanoodle.io/block/' + keyValue });
                            }
                            catch (err) {
                                console.log(err);
                            }
				...
				discordMessage(start, from, to, keyValue, summary.amount);
                            }
                            catch (err) {
                                console.log(err);
                            }
...
function discordMessage(start, from, to, keyValue, amount) {
                                client.login(process.env.DISCORD_TOKEN).then(function () {
                                    //nanoodle discord
                                    alertChannel = client.channels.get("604744766722408458");
                                    alertChannel.send(start + from + to + (+amount / 1000000000000000000000000000000).toFixed() + ' $NANO transfer -  https://nanoodle.io/block/' + keyValue);
                                    //nanotrade discord
...
function getMarketPrice(currencyType) {
    return new Promise(function (resolve, reject) {
        let xhr = new XMLHttpRequest();
        marketURL = 'http://localhost:8082/blocks/input?filter={"log.epochTimeStamp":{$gte: new Date(' + priceDate.getTime() + ')}}&filter={' + currencyType + ': {"$exists": true }}&np'
...
function getNewBlocks() {
    ...
    return new Promise(function (resolve, reject) {
        let xhr = new XMLHttpRequest();
        blockURL = 'http://localhost:8082/blocks/input?filter={"log.epochTimeStamp":{$gt: new Date(' + paramDate.getTime() + ')}}&filter={"is_send" : "true" }&keys={"hash" : 1 }&keys={"amount" : 1 }&sort={"log.epochTimeStamp" : -1}&pagesize=1000&np'
